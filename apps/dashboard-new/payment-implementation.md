...
Polar's standard recurring trial collects payment information during checkout. Marketing describes the offer as a 7-day Pro trial that can be canceled at any time; it does not claim that payment details are unnecessary. A no-card evaluation would require a separate local trial entitlement and is not part of this payment implementation.
...
Update `db_server.plan` transactionally after every subscription or grant change as a compatibility and operator-facing projection. The current rewritten bot does not consume this field, so paid-priority scheduling is not an acceptance criterion.
...
Generate a new timestamped Drizzle migration directory from the latest snapshot after preserving the user's existing untracked migration directories. Add a separate idempotent backfill script with dry-run counts, unresolved mappings, resumable batches, and no destructive rollback.
...
Emit one-line JSON billing events through the dashboard runtime's console logging path without full provider payloads. Vercel captures these records; active alert rules are a production operations task and must be configured before cutover rather than implied by application code.
