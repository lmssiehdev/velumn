...
import { DBChannel } from "@repo/db/schema/discord";
import { Guild } from "./_fetchUserGuilds";
...
export function OnboardingProvider({ guilds, initialChannels, initialGuildId }: { guilds: Guild[], initialChannels: SortChannel[], initialGuildId: string | null }) {
  const { user } = useAuth();
  console.log({ user })
  const [inviteUrl, setInviteUrl] = useState<string | null>(null);
  const [step, setStep] = useState<Step>(user.serverId ? "SELECT_CHANNELS" : "INVITING_SERVER");
  ...
  const value: OnboardingContextType = {
    ...
    toggleChannel,
    finishOnboarding: async (selectedChannels: string[]) => await finishOnBoardingMutation.mutateAsync({ userId: user.id, selectedChannels }),
      ...
      <div className="my-10 flex flex-col items-center justify-center">
        <div className="my-10 flex flex-col items-center justify-center">
          <div className="flex items-center justify-center whitespace-pre-line font-semibold text-3xl text-gray-800 leading-normal tracking-tight">
            Welcome to discord!{" "}
            <img alt="wave" className="ml-2 inline-block size-6" src={emojiToTwemoji("👋")} />
          </div>
...
function WaitingForBotToJoin() {
  const { selectedGuildId, guilds, setChannels, handleInviteCreation, inviteUrl } =
  ...
  }, [userQuery.isSuccess, userQuery.error, userQuery.data]);
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center">
        ...
        <div className="space-y-2">
          <Button onClick={() => handleInviteCreation(guildId)}>Re-invite Bot</Button>
            ...
            <div className="flex items-center justify-center gap-2 text-sm text-gray-500">
              ...
              {!userQuery.isFetching && (
                <span className="font-medium">
                  {Math.max(Math.floor((now - userQuery.dataUpdatedAt) / 1000), 0)} seconds ago
                </span>
              )}
            </div>
          )}
...
function GuildListItem({ guild }: { guild: Guild }) {
  const { step, selectGuild, handleInviteCreation, inviteUrl } = useOnboardingContext();
    ...
    <div className="flex items-center justify-between gap-2 rounded p-4 transition-all hover:bg-accent">
      <div className="flex items-center gap-4">
        <div className="flex aspect-square size-12 min-w-12 items-center justify-center overflow-hidden rounded-full bg-gray-100">
          {guild.icon ? (
            ...
            <div className="flex items-center font-bold">{initials.toUpperCase()}</div>
          )}
        </div>
        <div>
          <div>{guild.name}</div>
      ...
      </div>
      {
        isWaitingForBotToJoin && (
          !inviteUrl ? <Button variant={"outline"} className="rounded cursor-pointer" disabled>
            <CircleNotchIcon className="size-4 animate-spin" />
            Generating invite
          </Button> : <a href={inviteUrl} target="_blank" className={buttonVariants({ variant: "outline", className: "rounded cursor-pointer" })}>
            Invite Bot
          </a>
        )
      }
      ...
      <div className="mx-auto w-full max-w-md space-y-8">
                  ...
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={(value) => {
                      ...
                      setChannelFilterOptions((prevState) => {
                        ...
                        newState[prevState.findIndex((c) => c.type === column.type)].enabled =
                          value;
                        return newState;
                      });
                    }}
        ...
        <div className="my-1 flex items-center justify-between">
          <div>
            ...
            <div className="text-neutral-500 text-xs">(you can change this later)</div>
          </div>
          <Button className="flex items-center gap-2" onClick={() => finishOnboarding(selectedChannels)}>
            Start Indexing
          </Button>
        </div>
      </div>
...
function emojiToTwemoji(emoji: string, version = "14.0.2") {
  function toCodePoint(unicodeSurrogates: string) {
    ...
    while (i < unicodeSurrogates.length) {
      ...
      if (p) {
        r.push((0x1_00_00 + ((p - 0xd8_00) << 10) + (c - 0xdc_00)).toString(16));
        p = 0;
