...
export const serverRouter = router({
  ...
  finishOnboarding: privateProcedure
    .input(z.object({ userId: z.string(), selectedChannels: z.array(z.string()) }))
    .mutation(async ({ input, ctx }) => {
      try {
        const channels = input.selectedChannels.map(c => ({ channelId: c, status: true }))
  ...
  createServerInvite: privateProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        await createBotInvite({ serverId: input.serverId, userId: ctx.user.id });
