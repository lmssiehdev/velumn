...
const publicReferenceStarterAuthor = alias(
	dbDiscordUser,
	"public_reference_starter_author",
);
...
const publicBacklinkStarterAuthor = alias(
	dbDiscordUser,
	"public_backlink_starter_author",
);
	...
	return and(
		...
		exists(
			db
				...
				.innerJoin(
					publicStarterAuthor,
					eq(publicStarterMessage.authorId, publicStarterAuthor.id),
				)
				.where(
					and(
						...
						or(
							isNull(publicStarterAuthor.isIgnored),
							eq(publicStarterAuthor.isIgnored, false),
						),
					),
				),
		),
...
export async function resolveVerifiedPublicTenant(
	...
	return dedupeDatabaseRead(`public-tenant:${hostname}`, () =>
		queryVerifiedPublicTenant(hostname),
	);
}
...
async function queryPublicServer(
	serverId: string,
): Promise<PublicServerCapability | null> {
	const [server] = await db
		.select({ id: dbServer.id, name: dbServer.name })
...
async function queryPublicForumShell(
	...
	return {
		server: {
			...
			canonicalDomain: first.serverDomainVerified
				? first.serverCustomDomain
				: null,
		},
...
export async function resolveTenantPublicChannel(
	...
	return dedupeDatabaseRead(
		`public-channel:tenant:${tenantCapability.serverId}:${channelId}`,
...
export async function getPublicThreadMetadata(
	...
	return dedupeDatabaseRead(
		`public-thread-metadata:${capability.serverId}:${threadId}`,
...
async function queryPublicThreadMetadata(
	...
	const rows = await db
		...
		.leftJoin(
			publicThreadCategory,
			eq(publicThreadParent.parentId, publicThreadCategory.id),
		)
		.leftJoin(
			dbMessage,
			and(
	...
	if (
		!isPublicThreadVisible({
			...
			parentBelongsToServer:
				...
				row.serverId === row.threadServerId,
			parentIsUncategorized: row.parentParentId === null,
			parentCategoryBelongsToServer:
				row.parentCategoryServerId === row.threadServerId,
	...
	const messageCounts = db
		...
		.where(
			and(
				...
				eq(dbMessage.isIgnored, false),
				or(
					isNull(publicCountAuthor.isIgnored),
					eq(publicCountAuthor.isIgnored, false),
				),
			),
		)
...
function getPublicListAuthorName(row: {
	...
	return (
		webhook?.displayName?.trim() || webhook?.name?.trim() || row.authorName
	);
}
...
export async function getPublicThreadOgSummary(
	capability: PublicContentCapability | null,
	threadId: string,
	...
	return dedupeDatabaseRead(
		`public-thread-og:${capability?.serverId ?? "platform"}:${threadId}`,
		async () => {
			const [starterRows, countRows] = await Promise.all([
				db
					.select({
						title: dbChannel.channelName,
						parentName: publicThreadParent.channelName,
						serverName: dbServer.name,
						starterId: publicListStarter.id,
						starterContent: publicListStarter.content,
						starterEmbeds: publicListStarter.embeds,
						starterPoll: publicListStarter.poll,
						starterComponents: publicListStarter.components,
						starterSnapshot: publicListStarter.snapshot,
						starterStickers: publicListStarter.stickers,
					})
					.from(dbChannel)
					.innerJoin(dbServer, eq(dbChannel.serverId, dbServer.id))
					.innerJoin(
				...
				db
					...
					.innerJoin(
						dbMessage,
						and(
							eq(dbMessage.serverId, dbChannel.serverId),
							eq(dbMessage.primaryChannelId, dbChannel.id),
						),
					)
					.innerJoin(
						publicCountAuthor,
						eq(dbMessage.authorId, publicCountAuthor.id),
					)
					.where(
						and(
							publicThreadPredicate(capability?.serverId),
			...
			if (starterRows.length !== 1) return null;
			const [row] = starterRows;
			if (!row) return null;
			const starterContent = row.starterContent.trim();
			const hasRichBody = Boolean(
				starterContent ||
				row.starterEmbeds?.length ||
				row.starterPoll ||
				row.starterComponents?.length ||
				row.starterSnapshot ||
				row.starterStickers?.length,
			);
			if (!hasRichBody) {
				const attachmentRows = await db
					.select({ url: dbAttachments.proxyURL })
					.from(dbAttachments)
					.where(eq(dbAttachments.messageId, row.starterId));
				if (
					!attachmentRows.some((attachment) => getSafeHttpsUrl(attachment.url))
				)
					return null;
			}

			const title = row.title?.trim() || starterContent.slice(0, 100);
			if (!title) return null;
			const messageCount = countRows[0]?.count ?? 0;
			return {
				title,
				replyCount: Math.max(0, messageCount - 1),
				parent: { name: row.parentName ?? "Unknown channel" },
...
async function queryPublicThreadPage(
	...
	const rows = await db
		...
		.innerJoin(
			publicMessageAuthor,
			eq(dbMessage.authorId, publicMessageAuthor.id),
		)
		.where(
			and(
				publicThreadPredicate(capability?.serverId),
				...
				eq(dbMessage.isIgnored, false),
				or(
					isNull(publicMessageAuthor.isIgnored),
					eq(publicMessageAuthor.isIgnored, false),
				),
			),
		)
	...
	const mentionIds = collectPublicMentionIds([
		...mentionInputs,
		...snapshotMentionInputs,
	]);
	const referenceIds = [
		...new Set(
			pageRows.flatMap((row) =>
				row.messageReferenceId ? [row.messageReferenceId] : [],
			),
		),
	];
	const [
	...
	const privateMentionUserIds = new Set(
		userRows
			.filter((row) => row.redacted === true || row.anonymized)
			.map((row) => row.id),
	);
	...
	const starter = messages.find(
		(message) => message.id === starterRow.messageId,
	);
...
async function queryPublicReferences(
	...
	const rows = await db
			...
			hasVisibleStarter: exists(
				db
					...
					.innerJoin(
						...
						eq(
							publicReferenceStarter.authorId,
							publicReferenceStarterAuthor.id,
						),
					)
					.where(
						and(
							eq(
								publicReferenceStarter.primaryChannelId,
								publicReferenceThread.id,
							),
		...
		.innerJoin(
			publicReferenceAuthor,
			eq(publicReferenceMessage.authorId, publicReferenceAuthor.id),
		)
		.leftJoin(
		...
		.leftJoin(
			publicReferenceParent,
			eq(publicReferenceThread.parentId, publicReferenceParent.id),
		)
		.leftJoin(
		...
		.leftJoin(
			publicReferenceServer,
			eq(publicReferenceThread.serverId, publicReferenceServer.id),
		)
		.where(
			and(
...
async function queryPublicBacklinks(
	...
	const rows = await db
			...
			hasVisibleStarter: exists(
				db
					...
					.where(
						and(
							eq(
								publicBacklinkStarter.primaryChannelId,
								publicBacklinkThread.id,
							),
		...
		.innerJoin(
			publicBacklinkMessage,
			eq(dbThreadBacklink.fromMessageId, publicBacklinkMessage.id),
		)
		.innerJoin(
			publicBacklinkAuthor,
			eq(publicBacklinkMessage.authorId, publicBacklinkAuthor.id),
		)
		.innerJoin(
			publicBacklinkThread,
			eq(dbThreadBacklink.fromThreadId, publicBacklinkThread.id),
		)
		.innerJoin(
			publicBacklinkParent,
			eq(publicBacklinkThread.parentId, publicBacklinkParent.id),
		)
		.leftJoin(
			publicBacklinkCategory,
			eq(publicBacklinkParent.parentId, publicBacklinkCategory.id),
		)
		.innerJoin(
			publicBacklinkServer,
			eq(publicBacklinkThread.serverId, publicBacklinkServer.id),
		)
		.where(
			and(
				...
				eq(
					publicBacklinkMessage.primaryChannelId,
					dbThreadBacklink.fromThreadId,
				),
				eq(publicBacklinkMessage.serverId, publicBacklinkThread.serverId),
				eq(publicBacklinkMessage.isIgnored, false),
				or(
					isNull(publicBacklinkAuthor.isIgnored),
					eq(publicBacklinkAuthor.isIgnored, false),
				),
				...
				exists(
					db
						...
						.innerJoin(
							publicBacklinkStarterAuthor,
							eq(
								publicBacklinkStarter.authorId,
								publicBacklinkStarterAuthor.id,
							),
						)
						.where(
							and(
